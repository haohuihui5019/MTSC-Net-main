...
def parse_args():
    ...
    parser.add_argument('--data-dir', default='own_dataset',
...
if __name__ == '__main__':
    ...
    for phase in ['train', 'test', 'val']:
        ...
        if phase == 'train':
            im_list = sorted(glob(os.path.join(args.origin_dir, '1/train_data/images', "*.jpg")))
            sub_dir_den = os.path.join(sub_dir, 'gt_den')
            if not os.path.exists(sub_dir_den):
                os.makedirs(sub_dir_den)
        elif phase=='val':
            im_list = sorted(glob(os.path.join(args.origin_dir, 'val/images', "*.jpg")))
        else:
